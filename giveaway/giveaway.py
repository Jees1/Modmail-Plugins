...
class GiveawayPlugin(commands.Cog):
    ...
    async def _handle_giveaway(self, giveaway):
        ...
        while True:
            ...
            if g_time <= 0:
                ...
                embed = message.embeds[0]
                temp = embed.description.split("\n")
                temp[2] = f"Time Remaining: **{time_remaining}**"
                embed.description = "\n".join(temp)
                await message.edit(embed=embed)
                del channel, guild
                await asyncio.sleep(
    ...
    async def start(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        g_time = abs(giveaway_time - time.time())
        embed.description = (
            f"React with 🎉 to enter the giveaway!\n\n"
            f"Time Remaining: **{math.floor(g_time // 86400)} Days, {math.floor(g_time // 3600 % 24)} Hours, {math.floor(g_time // 60 % 60)} Minutes, {math.floor(g_time % 60)} Seconds** "
            f"\nHost: {ctx.author.mention} | {ctx.author.name}#{ctx.author.discriminator}"
        )
        embed.set_footer(
        ...
        msg: discord.Message = await channel.send("<@&736571375267086456>", embed=embed)
        ...
        new_embed.title = "New Giveaway"
        new_embed.description = (
            f"Drizzle Giveaways"
        )
        new_embed.set_author(name=ctx.author.name, icon_url=str(ctx.author.avatar_url))
        new_embed.color = self.bot.main_color
        
        new_embed.add_field(name="Host", value=f"{ctx.author.mention} | {ctx.author.name}#{ctx.author.discriminator}", inline=False)
        new_embed.add_field(name="Giveaway Item", value = giveaway_item.content, inline=False)
        new_embed.add_field(name="Winners", value = giveaway_winners, inline=False)
        new_embed.set_footer(text="Thanks to Piyush#4332")
        new_embed.set_thumbnail(url="https://media.discordapp.net/attachments/620497762072526879/748231653209407508/image0.png")
        logchannel = self.bot.get_channel(747190399164219402)
